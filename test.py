...
def test_imagenet(model, data_path, val_path, images_path, top_k=5):
    test_data = tf.placeholder(
        tf.float32,
        shape=(model.batch_size, model.crop_size, model.crop_size, model.channels))
...
def main():
    ...
    if len(args) not in (3, 4):
        print('usage: %s net.params imagenet-val.txt imagenet-data-dir [model-index=0]' %
